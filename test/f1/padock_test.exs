defmodule F1.PadockTest do
  ...
  describe "teams" do
    ...
    test "create_team/1 with valid data creates a team" do
      valid_attrs = %{base: "some base", car_image_url: "some car_image_url", chassis: "some chassis", entry: "some entry", fastest_lap: 42, highest_race_finish: "some highest_race_finish", name: "some name", pole_position: 42, power_unit: "some power_unit", team_chief: "some team_chief", team_image_url: "some team_image_url", technical_chief: "some technical_chief", world_championship: 42}

      assert {:ok, %Team{} = team} = Padock.create_team(valid_attrs)
      assert team.base == "some base"
      assert team.car_image_url == "some car_image_url"
      assert team.chassis == "some chassis"
      assert team.entry == "some entry"
      assert team.fastest_lap == 42
      assert team.highest_race_finish == "some highest_race_finish"
      ...
      assert team.power_unit == "some power_unit"
      assert team.team_chief == "some team_chief"
      assert team.team_image_url == "some team_image_url"
      assert team.technical_chief == "some technical_chief"
      assert team.world_championship == 42
    end
    ...
    test "update_team/2 with valid data updates the team" do
      team = team_fixture()
      update_attrs = %{base: "some updated base", car_image_url: "some updated car_image_url", chassis: "some updated chassis", entry: "some updated entry", fastest_lap: 43, highest_race_finish: "some updated highest_race_finish", name: "some updated name", pole_position: 43, power_unit: "some updated power_unit", team_chief: "some updated team_chief", team_image_url: "some updated team_image_url", technical_chief: "some updated technical_chief", world_championship: 43}

      assert {:ok, %Team{} = team} = Padock.update_team(team, update_attrs)
      assert team.base == "some updated base"
      assert team.car_image_url == "some updated car_image_url"
      assert team.chassis == "some updated chassis"
      assert team.entry == "some updated entry"
      assert team.fastest_lap == 43
      assert team.highest_race_finish == "some updated highest_race_finish"
      ...
      assert team.power_unit == "some updated power_unit"
      assert team.team_chief == "some updated team_chief"
      assert team.team_image_url == "some updated team_image_url"
      assert team.technical_chief == "some updated technical_chief"
      assert team.world_championship == 43
    end
